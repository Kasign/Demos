...
#define OrgLoopView_IOS
#else
#define OrgLoopView_MACOS
#endif


#ifdef OrgLoopView_IOS
...
typedef NS_ENUM(NSInteger, OrgLoopViewType)
{
    OrgLoopViewTypeLinear = 0,
    OrgLoopViewTypeRotary,
    OrgLoopViewTypeInvertedRotary,
    OrgLoopViewTypeCylinder,
    OrgLoopViewTypeInvertedCylinder,
    OrgLoopViewTypeWheel,
    OrgLoopViewTypeInvertedWheel,
    OrgLoopViewTypeCoverFlow,
    OrgLoopViewTypeCoverFlow2,
    OrgLoopViewTypeTimeMachine,
    OrgLoopViewTypeInvertedTimeMachine,
    OrgLoopViewTypeCustom
};


/**
 - OrgLoopViewOptionWrap: 一个布尔值，表示滚动条滚动到最后是否环绕。如果您希望carousel到达结束时环绕则返回YES，否则返回 NO.一般来说，循环carousel类型将默认为环绕，而线性转盘类型不会环绕。 不要担心返回类型是浮点值，因为除0.0之外的任何值都将被视为YES。
 - OrgLoopViewOptionShowBackfaces: 对于某些carousel类型，例如 OrgLoopViewTypeCylinder，可以看到某些视图的后侧（默认情况下，OrgLoopViewTypeInvertedCylinder隐藏背面）。如果您希望隐藏后视图，您可以为此选项返回 NO。要重写OrgLoopViewTypeInvertedCylinder的默认背景隐藏，可以返回YES。 此选项也可用于导致视图背面显示的自定义轮播转换。
 - OrgLoopViewOptionOffsetMultiplier: 用户用手指拖动转盘时使用的偏移倍数。它不影响程序滚动或减速速度。对于大多数轮播类型，默认值为1.0，但对于CoverFlow样式的 carousel，默认值为2.0，以补偿其项目间隔更紧密的事实，因此必须进一步拖动以移动相同的距离。
 - OrgLoopViewOptionVisibleItems: 这是一次应该在轮播中可见的视图（包括占位符）的最大数量。这一数量的一半视图将显示在当前所选项目索引的两边。超出该视图的视图在滚动到视图之前将不会被加载。这是考虑到carousel 中大量 item 会对性能产生不好的影响.。OrgLoopView根据轮播类型选择合适的默认值，如果您希望重写该值可以使用这个属性（例如，自定义的轮播类型）。
 - OrgLoopViewOptionCount: 在 Rotary,Cylinder 和 Wheel 类型中显示的 item 的数量。通常，这是根据轮播中的视图大小和item 的数量自动计算的，但如果要更精确地控制carousel的显示，则可以重写此选项。该属性用于计算 carousel半径，因此另一个选项是直接操作半径。
 - OrgLoopViewOptionArc: Rotary，Cylinder和Wheel 类型的弧线转换（弧度）。通常默认为2 * M_PI（一个完整的圆），但您可以指定较小的值，因此例如，M_PI的值将创建一个半圆或圆柱。该属性用于计算圆盘传送带半径和角度步长，因此另一个选项是直接操作这些值。
 - OrgLoopViewOptionAngle: 旋转，圆柱和轮的半径以像素/点转换。为了可见item显示的数量更好的适应特定的弧度,这个值通常被计算 。您可以操作该值来增加或减少项目间距（以及圆的半径）。
 - OrgLoopViewOptionRadius: Rotary，Cylinder和Wheel之间的每个项目之间的角度步长（以弧度表示）。 在不改变半径的情况下,在 carousel 滚动到最后时改变间隙或使item 交叠。
 - OrgLoopViewOptionTilt: tilt用于CoverFlow，CoverFlow2和TimeMachine的 carousel类型中的非居中项目。 该值应在0.0到1.0的范围内。
 - OrgLoopViewOptionSpacing: 项目视图之间的间距。该值乘以项目宽度（或高度，如果carousel 是垂直的），以获得每个项目之间的总空间，因此值为1.0（默认值）表示视图之间没有空隙（除非视图已经包括填充，因为 他们在许多示例项目中做）。
 - OrgLoopViewOptionFadeMin/OrgLoopViewOptionFadeMax/OrgLoopViewOptionFadeRange: 这三个选项根据它们与当前居中的item的偏移量来控制carousel的视图的淡出。FadeMin是item视图在开始淡入之前可以达到的最小负偏移量。 FadeMax是视图在开始淡化之前可以达到的最大正偏移量。FadeRange是项目可以在开始褪色的点与完全不可见的点之间移动的距离。
 - OrgLoopViewOptionFadeMinAlpha: 透明度
 */
typedef NS_ENUM(NSInteger, OrgLoopViewOption)
{
    OrgLoopViewOptionWrap = 0,
    OrgLoopViewOptionShowBackfaces,
    OrgLoopViewOptionOffsetMultiplier,
    OrgLoopViewOptionVisibleItems,
    OrgLoopViewOptionCount,
    OrgLoopViewOptionArc,
    OrgLoopViewOptionAngle,
    OrgLoopViewOptionRadius,
    OrgLoopViewOptionTilt,
    OrgLoopViewOptionSpacing,
    OrgLoopViewOptionFadeMin,
    OrgLoopViewOptionFadeMax,
    OrgLoopViewOptionFadeRange,
    OrgLoopViewOptionFadeMinAlpha
};


NS_ASSUME_NONNULL_BEGIN

@protocol OrgLoopViewDataSource, OrgLoopViewDelegate;

@interface OrgLoopView : UIView

@property (nonatomic, weak_delegate) IBOutlet __nullable id<OrgLoopViewDataSource> dataSource;
@property (nonatomic, weak_delegate) IBOutlet __nullable id<OrgLoopViewDelegate> delegate;
@property (nonatomic, assign) OrgLoopViewType type;
...
/**
 ...
 这个属性是只读的。
 如果你想重写这个默认值，
 实现carousel:valueForOption:withDefault:方法且给OrgLoopViewOptionWrap返回一个值。
 */
...
/**
 ...
 这个代理方法且给OrgLoopViewOptionVisibleItems返回一个值。
 */
...
@protocol OrgLoopViewDataSource <NSObject>

/*
 返回carousel中界面的数量
 */
- (NSInteger)numberOfItemsInCarousel:(OrgLoopView *)carousel;
 ...
 */
- (UIView *)carousel:(OrgLoopView *)carousel viewForItemAtIndex:(NSInteger)index reusingView:(nullable UIView *)view;
 ...
 */
- (NSInteger)numberOfPlaceholdersInCarousel:(OrgLoopView *)carousel;
 ...
 */
- (UIView *)carousel:(OrgLoopView *)carousel placeholderViewAtIndex:(NSInteger)index reusingView:(nullable UIView *)view;

@end


@protocol OrgLoopViewDelegate <NSObject>
@optional
 ...
 */
- (void)carouselWillBeginScrollingAnimation:(OrgLoopView *)carousel;

/*
 当carousel结束动画时，会调用此方法
 */
- (void)carouselDidEndScrollingAnimation:(OrgLoopView *)carousel;

/*
 无论何时滚动都会调用此方法。
 无论轮播是以编程方式滚动还是通过用户交互来调用它。
 */
- (void)carouselDidScroll:(OrgLoopView *)carousel;

/*
 当carousel滚动导致currentItemIndex属性更改足够大时，就会调用此方法。
 无论item 的 index 是按程序更新还是通过用户交互来调用。
 */
- (void)carouselCurrentItemIndexDidChange:(OrgLoopView *)carousel;
 ...
 */
- (void)carouselWillBeginDecelerating:(OrgLoopView *)carousel;
 ...
 */
- (void)carouselDidEndDecelerating:(OrgLoopView *)carousel;
 ...
 */
- (BOOL)carousel:(OrgLoopView *)carousel shouldSelectItemAtIndex:(NSInteger)index;

/*
 如果用户点击任何轮播项目视图（不包括占位符视图），包括当前选定的视图，此方法将触发。
 如果用户点击当前所选视图中的控件（即作为UIControl的子类的任何视图），此方法将不会触发。
 */
- (void)carousel:(OrgLoopView *)carousel didSelectItemAtIndex:(NSInteger)index;
...
/*
 ...
 - 如果你只想放大视图，那么最好使用OrgLoopViewOptionSpacing值。
 */
...
/*
 ...
 仅当轮播类型为OrgLoopViewTypeCustom时，才会调用此方法。
 */
- (CATransform3D)carousel:(OrgLoopView *)carousel itemTransformForOffset:(CGFloat)offset baseTransform:(CATransform3D)transform;
...
/*
 ...
 这些选项的含义列在下面的OrgLoopViewOption值下。
 在Options Demo中查看该方法的高级用法。
 */
- (CGFloat)carousel:(OrgLoopView *)carousel valueForOption:(OrgLoopViewOption)option withDefault:(CGFloat)value;
