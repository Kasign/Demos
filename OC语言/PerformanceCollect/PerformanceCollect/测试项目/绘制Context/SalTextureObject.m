...
- (void)changeTextureWithKey:(NSString *)key block:(TextureChangeBlock)block {
    ...
    if ([key isKindOfClass:[NSString class]]) {
        ...
        [SalImageDataCenter cache_imageResourceObjectWithStr:key block:^(SalLocalResourceObject * _Nonnull resourceObject, BOOL isUrl) {
            ...
            if (resourceObject) {
                ...
                if (strongSelf.oriImageInfo == nil) {
                    ...
                    strongSelf.oriImage = [SalImageDataCenter salImageWithData:strongSelf.oriImage];
                    showSize = strongSelf.oriImage.size;
                } else {
                    showSize = strongSelf.oriImageInfo.showSize;
                }
            } else if (!isUrl) {
                strongSelf.oriImage = [SalImageDataCenter salImageWithData:key];
                showSize = strongSelf.oriImage.size;
            } else {
                if (block) {
                    block(strongSelf, NO);
...
- (void)getOriImageInfoIfNeed {
    ...
    if (_oriImageInfo == nil) {
        ...
        SalLocalResourceObject * resourceObject = nil;
        BOOL needReSave = NO;
            ...
            if (resourceObject) {
                ...
                if (imageInfo == nil) {
                    if (_oriImage == nil) {
                        ...
                        _oriImage = [SalImageDataCenter salImageWithData:_oriImage];
                    }
                    needReSave = YES;
                }
            }
        ...
        if (needReSave) {
            ...
            if (_keyStorageType != SalStorageType_MEMORY && _keyStorageType != SalStorageType_NONE) {
                dispatch_async(dispatch_get_global_queue(0, 0), ^{
                    [SalStorageInstance saveData:resourceObject key:_key type:_keyStorageType];
                });
            }
        }
        CGSize showSize = imageInfo.showSize;
...
- (void)willDisplayTexture {
    ...
    if (!_isOriNil && !_isCropToNil) {
        if (self.steam.isSteam) {
            UIImage * image = [self.steam currentImage];
            texture = [SalTexture textureWithImage:image];
        } else if (_oriImage) {
            texture = [SalTexture textureWithImage:_oriImage];
            _oriImage = nil;
        }
    }
