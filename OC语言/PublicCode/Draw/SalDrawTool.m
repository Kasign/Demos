...
void SALChangePixBufferColor(uint8_t * pixBuffer, const CGFloat * colorComponents, BOOL needSmooth);
...
//更改图片的透明度
void SALChangeBufferOpacity(SALPixel_8888 * imageBuffer, size_t pixelNum, float opacity);
void SALChangePixBufferOpacity(uint8_t * pixBuffer, CGFloat opacity);
...
void DrawChangeWithDrawTask(CGContextRef context, SALPixel_8888 * imageBuffer, SALImageInfoStruct drawStruct, SalDrawTask * drawTask);

void DrawTaskWithScaleCTM(CGContextRef context, SALDrawOrientation orientation, void(^callback)(void));
void DrawTaskWithRotateCTM(CGContextRef context, SALDrawOrientation orientation, CGFloat angle, CGRect imageRect, CGRect boxRect);

void DrawWithDetailTask(CGContextRef context, SALPixel_8888 * drawBuffer, SALImageInfoStruct drawStruct, SalDetailDrawTask * detailTask, size_t drawScale);
void DrawBufferWithTask(SALPixel_8888 * drawBuffer, SALImageInfoStruct drawStruct, SalBufferTask * bufferTask, size_t drawScale);
...
void DrawPixBuffer(uint8_t * pixBufferD, uint8_t * pixBufferS, CGBlendMode mixModel);
...
void DrawChangeAllOpacity(CGContextRef context, SALPixel_8888 * imageBuffer, SALImageInfoStruct drawStruct, SalDrawTask * drawTask);
...
static SALPixel_8888 * _rgbImageBuf = NULL;
...
SalImageInfo * SALGetImageInfoWithTask(SalDrawTask * drawTask) {
    ...
    if ([drawTask isKindOfClass:[SalDrawTask class]] && drawTask.drawSize.width > 0 && drawTask.drawSize.height > 0) {
        ...
        if (drawTask.usePublicHeap) {
            if (_rgbImageBuf == NULL) {
                ...
                _rgbImageBuf = (SALPixel_8888 *)calloc(drawStruct.bytesPerPixel, size);
            }
...
void DrawWithDrawTask(CGContextRef context, SALPixel_8888 * imageBuffer, SALImageInfoStruct drawStruct, SalDrawTask * drawTask) {
    ...
    if ([drawTask isKindOfClass:[SalDrawTask class]]) {
        ...
        if (drawTask.backColor != nil) {
            DrawChangeAllColor(imageBuffer, drawStruct.pixelNum, drawTask.backColor);
        } else {
            if (drawTask.usePublicHeap) {
            ...
            if (drawTask.visibleRect.size.width > 0 && drawTask.visibleRect.size.height > 0) {
                for (SalDetailDrawTask * detailTask in drawTask.drawTaskList) {
                    if ([detailTask isEffective]) {
                        DrawWithDetailTask(context, imageBuffer, drawStruct, detailTask, drawTask.drawScale);
                    }
                }
            }
            
            //其他需要处理
            DrawChangeWithDrawTask(context, imageBuffer, drawStruct, drawTask);
        }
    }
}
...
void DrawWithDetailTask(CGContextRef context, SALPixel_8888 * drawBuffer, SALImageInfoStruct drawStruct, SalDetailDrawTask * detailTask, size_t drawScale) {
    ...
    if (detailTask.drawRect.size.width <= 0 || detailTask.drawRect.size.height <= 0) {
        return;
    }
    ...
    } else if ([detailTask isKindOfClass:[SalBufferTask class]]) {
        DrawBufferWithTask(drawBuffer, drawStruct, (SalBufferTask *)detailTask, drawScale);
    }
}

#pragma mark 绘制当前BufferTask
/// 直接绘制buffer到当前buffer
/// @param drawBuffer 当前绘制的buffer
/// @param drawStruct 绘制信息
/// @param bufferTask task
/// @param drawScale  缩放比例
void DrawBufferWithTask(SALPixel_8888 * drawBuffer, SALImageInfoStruct drawStruct, SalBufferTask * bufferTask, size_t drawScale) {
    
    if (bufferTask) {
        CGRect drawRect = bufferTask.drawRect;
        if (drawScale != 1) {
            drawRect = SALRectWithSacle(drawRect, drawScale);
        }
        
        //当前要绘制的buffer + 原buffer * （1 - 要绘制buffer.alpha）
        
        for (int i = 0; i < drawRect.size.width; i ++) {
...
void DrawImageWithTask(CGContextRef context, SALImageInfoStruct drawStruct, SalImageTask * imageTask, size_t drawScale) {
    ...
    if (imageTask) {
        ...
        CGRect drawBox   = imageTask.drawBox;
        if (drawScale != 1) {
            imageRect = SALRectWithSacle(imageRect, drawScale);
            drawBox   = SALRectWithSacle(drawBox, drawScale);
        }
        if (imageTask.zRotation != 0) {
            imageRect.origin.y = drawStruct.imageRefHeight - drawBox.origin.y - drawBox.size.height;
        } else {
            imageRect.origin.y = drawStruct.imageRefHeight - imageRect.origin.y - imageRect.size.height;
        }
        
        DrawTaskWithScaleCTM(context, imageTask.drawOrientation, ^{
            if (imageTask.zRotation != 0) {
                DrawTaskWithRotateCTM(context, imageTask.drawOrientation, imageTask.zRotation, imageRect, drawBox);
            }
...
void DrawTextWithTask(CGContextRef context, SALImageInfoStruct drawStruct, SalTextTask * textTask, size_t drawScale) {
    ...
    CTFramesetterRef framesetter = textTask.framesetterRef;
    // 创建绘制区域
...
//    NSLog(@"%@ suggest: %f %f true: %f %f", attributedStr.string, suggestedSize.width, suggestedSize.height, textRect.size.width, textRect.size.height);
    ...
    //整个区域绘制
    DrawTaskWithScaleCTM(context, textTask.drawOrientation, ^{
...
#pragma mark 绘制完成后集中处理

void DrawChangeWithDrawTask(CGContextRef context, SALPixel_8888 * imageBuffer, SALImageInfoStruct drawStruct, SalDrawTask * drawTask) {
    
    //把其他区域透明，只保留crop区域的颜色
    ...
    //改变所有颜色的透明度
    if (drawTask.opacity > 0 && drawTask.opacity < 1) {
        DrawChangeAllOpacity(context, imageBuffer, drawStruct, drawTask);
    }
}
...
void DrawTaskWithRotateCTM(CGContextRef context, SALDrawOrientation orientation, CGFloat angle, CGRect imageRect, CGRect boxRect) {
    ...
    if (angle != 0) {
        ...
        if (zRotation < 0) {
            zRotation = zRotation + M_PI * 2;
        }
        CGContextRotateCTM(context, zRotation);//绕着context左下角（0，0）点旋转，正值逆时针，负值顺时针（这里没有对context翻转）
        if (zRotation > 0 && zRotation < M_PI_2) {
            float mx = imageRect.size.height * sin(zRotation);
            tx = mx * cos(zRotation);
            ty = -mx * sin(zRotation);
        } else if (zRotation == M_PI_2) {
            tx = imageRect.size.height;
        } else if (zRotation == M_PI) {
            tx = imageRect.size.width;
            ty = imageRect.size.height;
        } else if (zRotation == M_PI_2 * 3) {
            ty = imageRect.size.width;
        } else if (zRotation > M_PI_2 && zRotation < M_PI) {
            float angle = zRotation - M_PI_2;
            float mx = imageRect.size.width * sin(angle);
            tx = mx * cos(angle) + imageRect.size.height;
            ty = -mx * sin(angle);
        } else if (zRotation > M_PI && zRotation < M_PI_2 * 3) {
            float angle = zRotation - M_PI;
            float mx = imageRect.size.height * sin(angle);
            tx = mx * cos(angle) + imageRect.size.width;
            ty = -mx * sin(angle);
        } else if (zRotation > M_PI_2 * 3 && zRotation < M_PI * 2) {
            float angle = zRotation - M_PI_2 * 3;
            float mx = imageRect.size.width * sin(angle);
            tx = mx * cos(angle);
            ty = -mx * sin(angle);
        }
...
void DrawChangeAllOpacity(CGContextRef context, SALPixel_8888 * imageBuffer, SALImageInfoStruct drawStruct, SalDrawTask * drawTask) {
    ...
    if (drawTask.drawSize.width > 0 && drawTask.drawSize.height > 0 && drawTask.visibleRect.size.width > 0 && drawTask.visibleRect.size.height > 0 && drawTask.opacity > 0 && drawTask.opacity < 1) {
        SALChangeBufferOpacity(imageBuffer, drawStruct.pixelNum, drawTask.opacity);
    }
}

/// 绘制源像素点到目标像素点上
/// @param pixBufferD 目标像素点（原来的像素点）
/// @param pixBufferS 源像素点（要绘制的像素点）
/// @param mixModel     绘制模式
void DrawPixBuffer(uint8_t * pixBufferD, uint8_t * pixBufferS, CGBlendMode mixModel) {
    
    if (mixModel == kCGBlendModeNormal) {
        size_t opacity = 1 - pixBufferS[0]/255.f;
        pixBufferD[0] = pixBufferD[0] * opacity + pixBufferS[0]; //alpha 0~255
        pixBufferD[1] = pixBufferD[1] * opacity + pixBufferS[1]; //blue  0~255
        pixBufferD[2] = pixBufferD[2] * opacity + pixBufferS[2]; //green 0~255
        pixBufferD[3] = pixBufferD[3] * opacity + pixBufferS[3]; //red   0~255
    }
}
...
void SALChangePixBufferOpacity(uint8_t * pixBuffer, CGFloat opacity) {
    ...
    pixBuffer[3] = pixBuffer[3] * opacity; //red   0~255
    pixBuffer[2] = pixBuffer[2] * opacity; //green 0~255
    pixBuffer[1] = pixBuffer[1] * opacity; //blue  0~255
    pixBuffer[0] = pixBuffer[0] * opacity; //alpha 0~255
}
            ...
            for (; y < cropH; y++) {//行数
                ...
                for (x = 0; x < cropW; x++, pCurPtr++) {
                    @autoreleasepool {
                        ...
                        SALChangePixBufferColor(ptr, colorComponents, NO);
                    }
                }
    ...
    if (imageBuffer && pixelNum > 0 && colorComponents != NULL) {
        ...
        for (size_t i = 0; i < pixelNum; i++, pCurPtr++) {
            @autoreleasepool {
                uint8_t * ptr = (uint8_t *)pCurPtr;
                if (ptr[0] > 0) {
                    SALChangePixBufferColor(ptr, colorComponents, YES);
                }
            }
        }
        pCurPtr = NULL;
...
void SALChangeBufferOpacity(SALPixel_8888 * imageBuffer, size_t pixelNum, float opacity) {
    ...
    if (imageBuffer && pixelNum > 0 && opacity != 1) {
        SALPixel_8888 * pCurPtr = imageBuffer;
        for (size_t i = 0; i < pixelNum; i++, pCurPtr++) {
            @autoreleasepool {
                uint8_t * ptr = (uint8_t *)pCurPtr;
                SALChangePixBufferOpacity(ptr, opacity);
            }
        }
        pCurPtr = NULL;
    }
...
- (void)setDrawRect:(CGRect)drawRect {
    ...
    _drawRect = drawRect;
    if (CGRectEqualToRect(_drawBox, CGRectZero)) {
        _drawBox  = drawRect;
    }
}
...
#pragma mark - SalBufferTask
@implementation SalBufferTask

+ (instancetype)bufferTaskWithImageInfo:(SalImageInfo *)imageInfo drawRect:(CGRect)drawRect {
    
    SalBufferTask * bufferTask = [SalBufferTask defaultTask];
    bufferTask.drawImageInfo   = imageInfo;
    bufferTask.drawRect        = drawRect;
    return bufferTask;
}
...
+ (instancetype)imageTaskWithObject:(id)imageObject drawRect:(CGRect)drawRect {
...
+ (instancetype)imageTaskWithImage:(UIImage *)image drawRect:(CGRect)drawRect {
    
    SalImageTask * imageTask = [SalImageTask defaultTask];
...
+ (instancetype)imageTaskWithImageRef:(CGImageRef)imageRef drawRect:(CGRect)drawRect {
...
+ (instancetype)imageTaskWithImageInfo:(SalImageInfo *)imageInfo drawRect:(CGRect)drawRect {
...
- (void)dealloc {
    ...
    if (_needRelease && _drawImageRef != NULL) {
        CGImageRelease(_drawImageRef);
    }
    _drawImageRef = NULL;
}
...
+ (SalTextTask *)textTaskWithAttributedStr:(NSAttributedString *)attributedStr drawRect:(CGRect)drawRect {
    ...
    SalTextTask * task = [SalTextTask defaultTask];
...
- (CTFramesetterRef)framesetterRef {
    ...
    if (_framesetterRef == NULL) {
        if (_attributedStr) {
            _framesetterRef = CTFramesetterCreateWithAttributedString((__bridge CFAttributedStringRef)_attributedStr);
        }
    }
...
+ (SalPathTask *)imageTaskWithPathRef:(CGPathRef)pathRef drawRect:(CGRect)drawRect {
    ...
    SalPathTask * pathTask = [SalPathTask defaultTask];
