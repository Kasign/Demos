...
- (void)encodeWithCoder:(NSCoder *)coder {
    ...
    if (_imageBuffer != NULL) {
        ...
        NSData * structData = nil;
        if (length > 0) {
            bufferData = [NSData dataWithBytes:_imageBuffer length:length];
            structData = [NSData dataWithBytes:&_imageStruct length:sizeof(SALImageInfoStruct)];
        }
        if (bufferData && structData) {
...
- (id)copyWithZone:(NSZone *)zone {
    ...
    if (self.imageStruct.pixelNum > 0 && self.imageBuffer != NULL && self.imageStruct.bytesPerPixel > 0) {
        imageInfo.imageBuffer = calloc(self.imageStruct.bytesPerPixel, self.imageStruct.pixelNum);
        if (imageInfo.imageBuffer != NULL) {
            memcpy(imageInfo.imageBuffer, self.imageBuffer, self.imageStruct.pixelNum * self.imageStruct.bytesPerPixel);
        }
    }
    return imageInfo;
}
