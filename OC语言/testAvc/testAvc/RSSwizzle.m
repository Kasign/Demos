...
static BOOL blockIsCompatibleWithMethodType(id block, const char *methodType){
    ...
    for (int i=0; i<methodSignature.numberOfArguments; ++i){
        if (i == 0){
            ...
            const char *methodSignatureArg = [methodSignature getArgumentTypeAtIndex:i];
            if (strncmp(methodSignatureArg, "^{NS", 4) == 0)
            {
                //函数签名对于OC原生类型存在不同描述方式，例如NSDictionary就存在 @<=>^{NSDictionary=#} 两种方式
                methodSignatureArg = "@";
            }
            
            if (strcmp(blockSignatureArg, methodSignatureArg) != 0)
            {
                return NO;
            }
        }
