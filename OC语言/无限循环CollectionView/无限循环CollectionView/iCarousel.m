...
{   
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeOrange:
        {
            CGFloat tilt    = [self valueForOption:iCarouselOptionTilt withDefault:0.5];
            CGFloat spacing = [self valueForOption:iCarouselOptionSpacing withDefault:0.1];
            //CGFloat tilt    = [self valueForOption:iCarouselOptionTilt withDefault:0];
            //CGFloat spacing = [self valueForOption:iCarouselOptionSpacing withDefault:0.24];
            ...
            CGFloat x = (clampedOffset * (0.24 - _numberOfItems / 100.f) * tilt + offset * spacing) * _itemWidth;
            //CGFloat x = (clampedOffset * 0.3 * tilt + offset * spacing) * _itemWidth;

            CGFloat count = [self circularCarouselItemCount];
            CGFloat spacing1 = [self valueForOption:iCarouselOptionSpacing withDefault:1.0];
            CGFloat arc = [self valueForOption:iCarouselOptionArc withDefault:M_PI * 2.0];
            CGFloat radius = [self valueForOption:iCarouselOptionRadius withDefault:MAX(_itemWidth * spacing / 2.0, _itemWidth * spacing1 / 2.0 / tan(arc/2.0/count))];
            CGFloat angle = [self valueForOption:iCarouselOptionAngle withDefault:offset / count * arc];
            
            //CGFloat _xxxx = offset * (self.frame.size.width / 2.f - (_itemWidth / 2.f - MXTransformW - MXTransformW * (10 - 15) / 10));
            //_xxxx = angle * 56;
            //_xxxx = [self translationByOffset:offset];
            //_xxxx = radius * sin(angle) - offset * (_itemWidth / 2.f + MXTransformW);
            CGFloat _zzzz = radius * cos(angle) - radius - 5;
            //CGFloat _zzzz = fabs(clampedOffset) * -_itemWidth * 0.5;
            return CATransform3DTranslate(transform, x, 0.0, _zzzz);
        }
        case iCarouselTypeRotary:
        case iCarouselTypeInvertedRotary:
        {
...
- (void)didScroll
{
    ...
    if (_previousItemIndex != self.currentItemIndex)
    {
        ...
        [self popAnimationState];
        [self mx_transformItemViewsIndex:self.currentItemIndex];
    }
...
- (void)mx_transformItemViewsIndex:(NSInteger)curIndex {
    
    for (NSNumber *number in _itemViews) {
        NSInteger index = [number integerValue];
        UIView *view = _itemViews[number];
        [self mx_transformItemView:view atIndex:index curIndex:curIndex];
    }
}

- (void)mx_transformItemView:(UIView *)view atIndex:(NSInteger)index curIndex:(NSInteger)curIndex {
    
//    if (index == curIndex) {
//        UIView * bgView = [view viewWithTag:2018 + index];
//        if (bgView) {
...
//            bgView = nil;
//        }
//        UIView * gbView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, view.frame.size.width, view.frame.size.height)];
//        gbView.backgroundColor = [UIColor yellowColor];
//        gbView.layer.cornerRadius = 8.f;
//        gbView.tag = 2018 + index;
    ...
    if (_scrollEnabled && _numberOfItems)
    {
        switch (panGesture.state)
        {
            ...
            case UIGestureRecognizerStateChanged:
            {
                ...
                if (velocity > 0) {
                    velocity = MIN(velocity, 1200.f);
                } else {
                    velocity = MAX(velocity, - 1200.f);
                }
                if (translation > 0) {
                    translation = MIN(translation, _itemWidth / 2.0);
                } else {
                    translation = MAX(translation, -_itemWidth / 2.0);
                }
                
                if (translation == _itemWidth / 2.0 || translation == -_itemWidth / 2.0) {
