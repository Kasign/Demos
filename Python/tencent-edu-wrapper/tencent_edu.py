...
class wrapper:
    ...
    def __dic_from_dic(self, dic):

        if dic:
            new_str0 = list(dic.keys())[0]
            new_str1 = list(dic.values())[0][0]
            
            # print(new_str0, type(new_str0), new_str1, type(new_str1))
            new_str2 = '{' + '\'' + new_str0 + '\'' + ':' + '\'' + new_str1 + '\'' + '}'
            new_str3 = new_str2.replace('=', '\':\'')
            new_str4 = new_str3.replace(';', '\',\'')
            print(new_str4)
            result_dic = eval(new_str4)
            print(result_dic)
            return result_dic
        return dic
    ...
    def __extract_from_url(self, url: str):
        ...
        if len(o_token_raw):
            ...
            # 转成字典
            if o_tokens:
                o_tokens = self.__dic_from_dic(o_tokens)
        else:
            o_tokens = None
    ...
    def __fetch_one_metadata(self, filename: str):
        ...
        result = ex1.fetchall()
        print('[+]\tMetadata of 111 {}:'.format(result))
        if len(result):
            metadata = result[0][1]
        else:
